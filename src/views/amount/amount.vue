<template>
    <div class="app-container">
        ...
        <!-- 分页主键 -->
        <pagination v-show="total > 10" v-model:page="queryParams.pageNum" v-model:limit="queryParams.pageSize" :total="total" @pagination="getAmountList" />
    </div>
</template>

<script setup name="Amount">
import { queryAmountList } from '@/api/amount'
import { getAmountCellData } from '@/dataSource/amount'
...
// 表格数据
const tableHeader = ref(getAmountCellData())
// 表头 查询条件
const queryTableParams = ref(getAmountCellData().filter(field => field.queryParameters))

/** 查询列表 */
function getAmountList() {
    loading.value = false
    queryAmountList(queryParams)
        .then(response => {
        ...
        .catch(error => {
            console.log('queryAmountList ~ error:', error)
            loading.value = true
        })
}
