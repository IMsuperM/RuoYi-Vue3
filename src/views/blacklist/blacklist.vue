        ...
        <el-dialog title="导入名单" v-model="open" @closed="reset" width="700px" style="margin-top: 20vh !important" append-to-body>
            <el-form :model="uploadForm" :show-message="false" ref="formRef" label-width="120px">
                ...
                <el-form-item label="名单类型">
                    <Radio :select-config="tag" :default-name="'黑名单'" :disabled="true" v-model:redioValue="uploadForm.tag" />
                </el-form-item>
...
const total = ref(0)
// 表格数据
const tableHeader = ref(getBlacklistCellData())
// 表头 查询条件
const queryTableParams = ref(getBlacklistCellData().filter(field => field.queryParameters))
